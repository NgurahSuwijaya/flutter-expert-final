...
class _MovieDetailPageState extends State<MovieDetailPage> {
  ...
  void initState() {
    super.initState();
    Future.microtask(() {
      context.read<MovieDetailBloc>().add(OnMovieDetailEvent(widget.id));
      context.read<RecommendationMoviesBloc>()
          .add(OnRecommendationMoviesEvent(widget.id));
      context.read<WatchlistMovieBloc>().add(OnTapWatchlistMovie(widget.id));
    });
  }
  ...
  Widget build(BuildContext context) {
    final isAddedToWatchlist = context.select<WatchlistMovieBloc, bool>((bloc) {
      if (bloc.state is InsertMovieToWatchlist) {
        return (bloc.state as InsertMovieToWatchlist).watchlistStatus;
      }
      return false;
    });
    return Scaffold(
